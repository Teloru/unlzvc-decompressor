...
def extract_dialogue(text):
    """Extract all dialogue lines from the text, grouped by source file"""
    lines = text.split('\n')
    dialogue_groups = {}  # file_name -> list of dialogues
    current_file = "Unknown"
    dialogue_groups[current_file] = []
    ...
    for line in lines:
        ...
        if line.startswith('=== FILE:'):
            # Extract filename from "=== FILE: .\JUNKYARD.IFF ===" or "=== FILE: ..\JUNKYARD.IFF ==="
            file_match = re.search(r'=== FILE: (.+?) ===', line)
            if file_match:
                file_path = file_match.group(1)
                # extract filename without path and clean prefixes
                current_file = file_path.split('/')[-1].split('\\')[-1]
                # remove path prefixes if they exist
                if current_file.startswith('.\\'):
                    current_file = current_file[2:]
                elif current_file.startswith('..\\'):
                    current_file = current_file[3:]
                if current_file not in dialogue_groups:
                    dialogue_groups[current_file] = []
            continue
            ...
            if data_content and data_content != "Data is empty":
                ...
                for extracted in extracted_list:
                    if extracted:
                        ...
                        if formatted:
                            dialogue_groups[current_file].append(formatted)
        else:
            if current_file not in dialogue_groups:
                dialogue_groups[current_file] = []
...
def main():
    ...
    try:
        ...
        print(f"Extracted {total_dialogues} dialogues from {len(dialogue_groups)} files to {output_file}")
